...
int sd_read_csv_ver1(const char *filename, CsvRecord1 *records, int max_records, int *record_count) {
	FIL file;
	char line[128];
	*record_count = 0;
	...
	while (f_gets(line, sizeof(line), &file) && *record_count < max_records) {
		...
		token = strtok(NULL, ",");
		if (!token) continue;
		strncpy(records[*record_count].field3, token, sizeof(records[*record_count].field3));
		token = strtok(NULL, ",");
		if (!token) continue;
		strncpy(records[*record_count].field4, token, sizeof(records[*record_count].field4));
